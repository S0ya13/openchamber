...
Budget the close against the real window, which differs by an order of magnitude between platforms. Measured: 120 to 190 ms on Linux with an idle profile, 1.8 to 2.3 seconds on Windows. On Linux that ruled out anything slow: a click on the app's own close button costs about 200 ms through the renderer and lands after the handover every time, while the menu's close role handled in the main process costs about 62 ms and fits. Spawning the input tool per click can itself cost 100 ms, so keep it warm.
