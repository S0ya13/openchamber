...
4. **Evaluate the contribution contract.** Verify that the PR explains its intent and scope, provides current, proportionate validation, and includes each screenshot, interaction recording, empirical measurement, and live-run statement the change requires, which stack rather than substitute for one another (see *Contribution quality and evidence*).
...
This requirement is additional, never a substitute. A visual change needs its screenshots **and** a live-run statement: a screenshot proves what the surface looks like, a live-run statement says a person reached that surface in a running build. Neither answers the other's question, so accepting one in place of the other is an error. The same holds for recordings and measurements.
